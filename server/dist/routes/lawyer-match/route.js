...
LawyerMatch.post('/lawyer-match', async (c) => {
    try {
        const { severity, violenceType, location, childrenInvolved, urgency, languages, preferredGender } = await c.req.json();
        ...
        if (languages && languages.length > 0) {
            const languageMatches = matchedLawyers.filter((lawyer) => languages.some((lang) => lawyer.languages.includes(lang)));
        ...
        if (severity === "critical" || urgency === "immediate") {
            ...
            matchedLawyers.sort((a, b) => {
                ...
                if (a.availability.includes("immediately") && !b.availability.includes("immediately"))
                    return -1;
                if (!a.availability.includes("immediately") && b.availability.includes("immediately"))
                    return 1;
            ...
            matchedLawyers.sort((a, b) => {
                ...
                if (a.availability.includes("24 hours") && !b.availability.includes("24 hours"))
                    return -1;
                if (!a.availability.includes("24 hours") && b.availability.includes("24 hours"))
                    return 1;
                return 0;
