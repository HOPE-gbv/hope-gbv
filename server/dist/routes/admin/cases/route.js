...
const serializeCaseDates = (caseItem) => {
    ...
    if (caseItem.notes) {
        caseItem.notes = caseItem.notes.map((note) => ({
            ...note,
            date: note.date.toISOString(),
        }));
    }
    if (caseItem.actions) {
        caseItem.actions = caseItem.actions.map((action) => ({
            ...action,
...
const deserializeCaseDates = (caseData) => {
    ...
    if (caseData.actions) {
        caseData.actions = caseData.actions.map((action) => ({
            ...action,
...
Admincases.get('/api/admin/cases', async (c) => {
    try {
...
Admincases.get('/api/admin/cases/:id', async (c) => {
    try {
...
Admincases.post('/api/admin/cases', async (c) => {
    try {
        ...
        const newCase = await db.case.create({
            data: {
                ...
                notes: {
                    create: deserializedCaseData.notes?.map((note) => ({
                        date: note.date,
                        author: note.author,
                ...
                actions: {
                    create: deserializedCaseData.actions?.map((action) => ({
                        type: action.type,
...
Admincases.put('/api/admin/cases/:id', async (c) => {
    try {
        const caseId = c.req.param("id");
        ...
        const updatedCase = await db.case.update({
            ...
            data: {
                ...
                notes: {
                    ...
                    create: deserializedUpdates.notes?.map((note) => ({
                        date: note.date,
                        author: note.author,
                        content: note.content,
                    })) || [],
                ...
                actions: {
                    ...
                    create: deserializedUpdates.actions?.map((action) => ({
                        type: action.type,
                        status: action.status,
                        dueDate: action.dueDate,
                        assignedTo: action.assignedTo,
...
Admincases.delete('/api/admin/cases/:id', async (c) => {
    try {
        const caseId = c.req.param("id");
