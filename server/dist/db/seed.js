// seed.ts
import { LAWYERS, CASES, REPORTS, EMERGENCY_CONTACTS, ADMIN_USERS, RESOURCES, } from '../data/config.js';
...
async function seedLawyers() {
    ...
    for (const lawyerData of LAWYERS) {
        ...
        // Correct seeding for `specializations` and `languages`
        const lawyer = await prisma.lawyer.upsert({
            where: { barNumber: rest.barNumber },
            update: {
                ...rest,
                specializations: rest.specializations, // Ensure correct type
                languages: rest.languages, // Ensure correct type
            },
            create: {
                ...rest,
                specializations: rest.specializations, // Ensure correct type
        ...
        // Efficiently create related data (after the lawyer is created)
        if (testimonials && testimonials.length > 0) {
            await prisma.lawyerTestimonial.createMany({
                data: testimonials.map(t => ({ ...t, lawyerId: lawyer.id })),
                skipDuplicates: true, // Avoid re-inserting existing data
            });
        }
        if (expertise && expertise.length > 0) {
            await prisma.lawyerExpertise.createMany({
                data: expertise.map((e) => ({ ...e, lawyerId: lawyer.id })),
                skipDuplicates: true,
            });
        }
        ...
        if (availabilityCalendar && availabilityCalendar.length > 0) {
            await prisma.lawyerAvailability.createMany({
                data: availabilityCalendar.map(a => ({
                    ...a,
                    date: new Date(a.date), // Convert date to Date object
                    slots: a.slots, // Ensure correct type
                    lawyerId: lawyer.id,
                })),
                skipDuplicates: true,
            });
...
async function seedCases() {
    ...
    for (const caseData of CASES) {
        ...
        const caseRecord = await prisma.case.upsert({
            ...
            update: {
                ...rest,
                reportDate: new Date(rest.reportDate), // Convert date string to Date object
                lastUpdated: new Date(rest.lastUpdated), // Convert date string to Date object
            },
            create: {
                ...rest,
                reportDate: new Date(rest.reportDate), // Convert date string to Date object
...
async function seedReports() {
    ...
    for (const reportData of REPORTS) {
        await prisma.report.upsert({
            ...
            update: { ...reportData, reportDate: new Date(reportData.reportDate) }, // Convert date
            create: {
                ...reportData,
                reportDate: new Date(reportData.reportDate), // Convert date
            },
        });
    }
...
async function seedEmergencyContacts() {
    ...
    for (const category of Object.keys(EMERGENCY_CONTACTS)) { // Type assertion
        if (category === 'stateContacts') {
            for (const region of Object.keys(EMERGENCY_CONTACTS[category])) { // Type assertion
                for (const contact of EMERGENCY_CONTACTS[category][region]) { // Type assertion
                    contactsToSeed.push({ ...contact, region });
                }
            }
        }
        else {
            for (const contact of EMERGENCY_CONTACTS[category]) { // Type assertion
...
async function seedResources() {
    ...
    for (const category of Object.keys(RESOURCES)) { // Type assertion
        for (const resource of RESOURCES[category]) { // Type assertion
